...
def cross_validation_experiment(plot_graph=True):
    ...
    # ====== YOUR CODE: ======
    m_choices = [0,1,3,5,7,9,13,17,25]
    max_accuracy = 0
    ...
    kf = KFold(n_splits=num_folds, shuffle=True, random_state=ID1)  # Using the provided random_state
    for m in m_choices:
        fold_accuracies_for_m = []

        for fold, (train_index, val_index) in enumerate(kf.split(x_train, y_train)):
            X_train_fold, X_val_fold = x_train[train_index], x_train[val_index]
            y_train_fold, y_val_fold = y_train[train_index], y_train[val_index]

            dt = ID3("diagnosis",min_for_pruning=m)
            dt.fit(X_train_fold, y_train_fold)
            y_pred_val = dt.predict(X_val_fold)
            fold_accuracy = accuracy(y_val_fold, y_pred_val)
            fold_accuracies_for_m.append(fold_accuracy)
            if np.mean(fold_accuracies_for_m) >= max_accuracy:
                best_m=m
                max_accuracy=np.mean(fold_accuracies_for_m)

        accuracies.append(fold_accuracies_for_m)
    # ========================
...
def best_m_test(x_train, y_train, x_test, y_test, min_for_pruning):
    ...
    # ====== YOUR CODE: ======
    dt=ID3("diagnosis",min_for_pruning)
    dt.fit(x_train, y_train)
